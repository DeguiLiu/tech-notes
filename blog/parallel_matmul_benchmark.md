...
测试环境: 512 x 512 float 矩阵，64 线程/进程，Release -O3。
...
cmake .. -DCMAKE_BUILD_TYPE=Release -DMATRIX_DIM=512
make -j$(nproc)
...
依赖: newosp 通过 CMake FetchContent 自动拉取，无需手动安装。
...
并行方案的选择不是"哪个最快"，而是"哪个最适合你的架构约束"。裸线程最快但最脆弱，WorkerPool 平衡了性能和工程质量，SHM 多进程提供了最强的隔离性。
