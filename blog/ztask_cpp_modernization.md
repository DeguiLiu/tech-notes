# ztask-cpp: 嵌入式合作式任务调度器的 C++14 现代化重写
...
> 在裸机 MCU 和轻量 RTOS 项目中，合作式任务调度器是比操作系统线程更轻量的执行抽象。原始 C 版 ztask 用运行时内存池 + 指针链表实现了这一机制，但存在类型安全缺失、无 ABA 防护、内存池需手动对齐等问题。本文以 ztask-cpp 为例，展示如何用 C++14 模板将这些运行时约束提升为编译期保证，在不增加运行时开销的前提下获得更强的安全性和 5-7% 的性能提升。

## 1. 背景: 为什么要重写

原始 C 版 ztask 的调度算法是成熟的 -- 排序链表头部检查实现 O(1) poll，`zt_ticks_to_next_task()` 提供精确休眠计算。但它的工程实现存在三个结构性问题:
...
int32_t num = zt_init(zt_mem, sizeof(zt_mem));
```

问题:
- `sizeof(struct zt_task_s)` 是不透明的 -- 用户需要包含内部头文件才能获得正确的大小
- 对齐依赖 `__attribute__((aligned(4)))` 而非类型系统保证
- `zt_init` 的返回值需要运行时检查，容量错误延迟到运行期才暴露

### 1.2 `void*` 回调的类型安全缺失

```c
typedef void (*zt_func_t)(void);  // 无参回调，无 context
zt_bind(task_a, 100, 50);         // context 只能通过全局变量传递
```

原始 ztask 的回调签名是 `void(void)`，不支持 context 指针。多个任务共享同一个回调函数时，只能通过全局变量区分状态 -- 这在多实例场景下不可维护。

### 1.3 无 ABA 防护

```c
zt_task_handle_t handle = zt_bind(my_task, 100, 0);
zt_unbind(handle);        // slot 被释放
zt_bind(other_task, 50, 0);  // slot 被复用
zt_unbind(handle);        // 悬挂引用! 可能误解绑 other_task
```

C 版返回的 handle 是裸指针 (`zt_task_s*`)，unbind 后 slot 被回收到 free list，新任务可能分配到同一 slot。旧 handle 仍然指向有效内存，`zt_unbind` 无法检测出这是一个过期引用。

## 2. 架构设计

ztask-cpp 保留了原始算法的核心 -- 排序侵入式链表，但将运行时约束提升为编译期保证:
...
```cpp
static ztask::TaskScheduler<8> sched;
...
| 编译期已知任务数 | ztask-cpp |
| 需要 ABA 安全 | ztask-cpp |
| 多实例 context 传递 | ztask-cpp |
| 纯 C 代码库 | C 版 ztask |
| 运行时动态任务数 | C 版 ztask |
| 极致代码体积优先 | C 版 ztask |

两个版本共享相同的调度算法内核。选择取决于项目语言环境和安全性需求，而非性能 -- 两者在热路径上的差异仅 5-7%。
